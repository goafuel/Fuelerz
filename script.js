...
document.getElementById('fuelerz-order-form').addEventListener('submit', function(e) {
    ...
    // Process form data
    const formData = {
        // ... (same as previous form data collection)
    };
    
    // Create WhatsApp message
    const whatsappMessage = `Thank you for your order!%0A%0A` +
                           `Here are your submitted details:%0A%0A` +
                           `*Name:* ${formData.name}%0A` +
                           `*Service:* ${formData.service}%0A` +
                           `*Budget:* ${formData.budget}%0A%0A` +
                           `*Project Details:*%0A${formData.message}%0A%0A` +
                           `We'll contact you within 24 hours. For immediate assistance, call +91 7722011476`;
    
    // Send to customer's WhatsApp
    const customerWhatsappUrl = `https://wa.me/91${formData.phone}?text=${whatsappMessage}`;
    
    // Send to your WhatsApp (business)
    const businessWhatsappUrl = `https://wa.me/917722011476?text=${encodeURIComponent(
        `NEW ORDER from website:%0A%0A` +
        `Name: ${formData.name}%0A` +
        `Phone: ${formData.phone}%0A` +
        `Service: ${formData.service}%0A` +
        `Budget: ${formData.budget}`
